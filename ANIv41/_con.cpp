...
struct anim_header *p_anim_header = NULL;
struct net_node *p_net_node = NULL;
...
int action_features_reply (char *of_buffer)
{
    ...
    if (pthread_mutex_lock (&links_mutex) == 0) {
        ...
        if (pthread_create (&node_send_thread, NULL, node_share_func, NULL) < 0) cerr << "Error: can not create thread for node sharing"<< endl;
...
int action_packout (char *of_buffer)
{
    ...
    if (ntohs(p_packet_in_eth->type) == IP6_DATA) {   
        ...
        p_packet_in_eth = (struct of_ethernet *) p_packet_in->data;
        p_packet_in_eth++;
        p_anim_header = (struct anim_header *) p_packet_in_eth;
        if (pthread_mutex_lock (&links_mutex) == 0) {
            if (p_anim_header->type == ANIM_ECHO_REQUEST) {
                ...
                struct anim_header e_anim_header;
                char eth_buffer[1024];
                char n_buffer[10240];
                long sw_write_len = -1;

                cout << "Info: received echo request from in port " << in_port << endl;
                
                memset(n_buffer, '\0', sizeof (n_buffer));
                memset(eth_buffer, '\0', sizeof (eth_buffer));
                memset(&e_anim_header, '\0', sizeof (e_anim_header));
                ...
                n_packet_out->in_port = htons (65533);
                ...
                n_action_output->port = htons (in_port);
                memcpy (n_buffer + sizeof (struct ofp_packet_out) + sizeof (struct ofp_action_output), eth_buffer, sizeof (struct of_ethernet));

                e_anim_header.type = ANIM_ECHO_REPLY;
                e_anim_header.length = htons (sizeof(e_anim_header));

                memcpy (n_buffer + sizeof (struct ofp_packet_out) + sizeof (struct ofp_action_output) + sizeof (struct of_ethernet), &e_anim_header, sizeof(e_anim_header));
                n_packet_out->header.length = htons (sizeof (struct ofp_packet_out) + sizeof (struct ofp_action_output) + sizeof (struct of_ethernet) + sizeof(e_anim_header));
                sw_write_len = send(sw_sockfd, n_buffer, ntohs (n_packet_out->header.length), 0);

                if (sw_write_len < 1) cerr << "Error: can not send echo reply to port " << in_port << ", " << strerror(errno) << endl;
                else cout << "Info: send echo reply to port " << in_port << endl;
            }
            else if (p_anim_header->type == ANIM_ECHO_REPLY) {
                struct switch_reply *p = NULL;
                cout << "Info: received echo reply from in port " << in_port << endl;
                
                p = (struct switch_reply *) sw_replys.add_object ();
                if (p == NULL) cerr << "Error: can not create object in switch reply table" << endl;
                else {
                    ((struct switch_reply *)p)->port_no = in_port;
                    ((struct switch_reply *)p)->rec = true;
                }
            }
            else if (p_anim_header->type == ANIM_NODE) {
                p_anim_header++;
                p_net_node = (struct net_node *) p_anim_header;
                struct switch_link *p = NULL;
                p = (struct switch_link *) links.add_object ();
                if (p == NULL) cerr << "Error: can not create object in links table" << endl;
                else {
                    ((struct switch_link *)p)->node_a.datapath_id = switch_id;
                    ((struct switch_link *)p)->node_a.port_no = in_port;
                    ((struct switch_link *)p)->node_b.datapath_id = be64toh(p_net_node->datapath_id);
                    ((struct switch_link *)p)->node_b.port_no = ntohs(p_net_node->port_no);
                }
                anim_node_counter = 0;
            }
            pthread_mutex_unlock (&links_mutex);
...
void *node_share_func (void *message)
{
    ...
    for (int portno = 1; portno < (ports_count + 1); portno++) {
        ...
        e_anim_header.type = ANIM_ECHO_REQUEST;
        e_anim_header.length = htons (sizeof(e_anim_header));
        ...
        if (sw_write_len < 1) cerr << "Error: can not send echo to port " << portno << ", " << strerror(errno) << endl;
        else cout << "Info: send echo to port " << portno << endl;
        sleep (5);
    }
    
    if (pthread_mutex_lock (&links_mutex) == 0) {
        struct switch_reply *p = NULL;
        p = (struct switch_reply *) sw_replys.get_object();
        while (p != NULL) {
            struct ofp_packet_out *n_packet_out = NULL;
            struct ofp_action_output *n_action_output = NULL;
            struct of_ethernet *n_packet_out_eth = NULL;
            struct anim_header n_anim_header;
            struct net_node n_anim_node;
            char eth_buffer[1024];
            char n_buffer[10240];
            long sw_write_len = -1;
            ...
            memset(n_buffer, '\0', sizeof (n_buffer));
            memset(eth_buffer, '\0', sizeof (eth_buffer));
            memset(&n_anim_header, '\0', sizeof (n_anim_header));
            memset(&n_anim_node, '\0', sizeof (n_anim_node));
            ...
            n_action_output->max_len = htons (0);
            n_action_output->len = htons (8);
            n_action_output->port = htons (((struct switch_reply *)p)->port_no);
            memcpy (n_buffer + sizeof (struct ofp_packet_out) + sizeof (struct ofp_action_output), eth_buffer, sizeof (struct of_ethernet));

            n_anim_header.type = ANIM_NODE;
            n_anim_header.length = htons (sizeof(n_anim_header) + sizeof(n_anim_node));
            n_anim_node.datapath_id = htobe64 (switch_id);
            n_anim_node.port_no = htons (((struct switch_reply *)p)->port_no);

            memcpy (n_buffer + sizeof (struct ofp_packet_out) + sizeof (struct ofp_action_output) + sizeof (struct of_ethernet), &n_anim_header, sizeof(n_anim_header));
            memcpy (n_buffer + sizeof (struct ofp_packet_out) + sizeof (struct ofp_action_output) + sizeof (struct of_ethernet) + sizeof (n_anim_header), &n_anim_node, sizeof(n_anim_node));
            n_packet_out->header.length = htons (sizeof (struct ofp_packet_out) + sizeof (struct ofp_action_output) + sizeof (struct of_ethernet) + sizeof(n_anim_header) + sizeof (n_anim_node));
            sw_write_len = send(sw_sockfd, n_buffer, ntohs (n_packet_out->header.length), 0);

            if (sw_write_len < 1) cerr << "Error: can not send node " << ((struct switch_reply *)p)->port_no << ", " << strerror(errno) << endl;
            else cout << "Info: send node to port " << ((struct switch_reply *)p)->port_no << endl;
            p = (struct switch_reply *) sw_replys.get_object();
        }
        sw_replys.list_rewind();
        pthread_mutex_unlock (&links_mutex);
    }
    ...
    if (pthread_mutex_lock (&links_mutex) == 0) {
        struct switch_link *pl = NULL;
        cout << "Switch Links" << endl;
        pl = (struct switch_link *) links.get_object();
        while (pl != NULL) {
            cout << "Datapath " << pl->node_a.datapath_id << " with port " << pl->node_a.port_no << " is connected to datapath " << pl->node_b.datapath_id << " with port " << pl->node_b.port_no << endl;
            pl = (struct switch_link *) links.get_object();
        }
        links.list_rewind();
        sw_replys.free_list();
